...
# the callback can't be CSRF-confused. In-memory dict is fine for Phase 1
# (single-instance) — same caveat as the other stores.
...
async def sso_login():
    ...
    _state_cache[state] = "pending"
    return RedirectResponse(url=ms_oauth.build_authorize_url(state))
...
async def sso_callback(code: str | None = None, state: str | None = None):
    ...
    # CSRF: reject unknown/absent state.
    if not state or _state_cache.pop(state, None) is None:
    ...
    try:
        tokens = ms_oauth.exchange_code_for_tokens(code)
        claims = ms_oauth.decode_id_token(tokens["id_token"])
    except Exception:
