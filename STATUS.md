...
Last updated: session 13 (eight-phase deep-review fix pass — all H1–H12 findings plus the medium/low items).

Legend: ✅ Done & verified (executed) · 🟡 Code complete, not executable-without-a-live-dependency · ⬜ Not started · ⚠️ Flagged issue/gap

## Session 13: deep-review fix pass (8 phases)

Close-out of the full in-depth review (H1–H12 + medium/low findings). Every
phase is code-complete and tracked with tests; backends suite is **131/131**,
frontend **12/12**, `vite build` green, `docker compose config` exit 0.

### ✅ Phase 1 — Agent builds pin to server versions

- Downgraded the agent bundle to match the servers it registers against:
  **Wazuh 4.9.0** (`wazuh-agent-4.9.0-1.msi`) and **Zabbix 6.4.20**
  (`zabbix_agent2-6.4.20-windows-amd64-openssl.msi`) in `AgentConfig.sample.json`,
  `provision_secrets.py`, `setup.py`, the Actions workflow, `fetch-agent-build.sh`,
  and `downloads.py` (both URLs live-verified HTTP 200 at those versions).
- `build-agent-installer.ps1` downloads the vendor agents under canonical
  unversioned names (`wazuh-agent.msi`, `zabbix-agent.msi`,
  `meshcentral-agent.exe`) and gained `-d IncludeMesh=1/0`.
- `MESHCENTRAL_MESH_ID` is now a first-class non-secret key (generated by
  `generate-secrets.sh`, blank in `blank_env`, optional `mesh_id` field in the
  wizard), read from `env or env_updates`.

### ✅ Phase 2 — TLS/URL/env hygiene

- Wizard writes `WAZUH_API_USER=aaditech-portal-svc` (H7); `GLPI_API_URL` set
  to `http://glpi:80` (H6).
- Unified TLS policy: `settings.tls_verify()` against the mounted mkcert
  `rootCA.pem` (`AADITECH_CA_CERT_PATH=/certs/rootCA.pem`), threaded through
  all four clients (Wazuh, MeshCentral, Zabbix, GLPI); client error messages no
  longer embed raw `resp.text`.
- `portal-logs` named volume mounted at `/var/log/aaditech` on backend + setup.
- `_upsert_env`/`_load_env` now quote values containing `#`/`=`/spaces
  (roundtrip test covers `COMPANY_NAME="Acme Corp"`).
- `ms_oauth` authorize/token-exchange take a request-derived base URL.
- `/api/setup/provision` gated by `setup_mode` (403 otherwise).
- Removed dead settings `portal_env`, `portal_ip`, `wazuh_enroll_key`
  (config.py + compose); `company_name` kept.

### ✅ Phase 3 — Security hardening

- **H8**: SSO JWT delivered as a URL **fragment** (`/login#token=…`), not a
  query param — kept out of proxy/access logs and the Referer header;
  `Login.jsx` reads the hash (legacy query fallback kept).
- **H4**: `require_service_token` dependency (JWT must carry `service: true`);
  `POST /cleanup/purge-expired` now refuses anonymous AND ordinary user
  sessions — only `create_service_token` credentials pass.
- **3.3**: `ms_oauth.verify_id_token` — signature against the tenant JWKS
  (kid-matched, cached 1h), audience, issuer, expiry and **nonce** (replay);
  the SSO router mints+stores the nonce and verifies it on callback.
- **3.5**: in-memory brute-force guard on local login (5 failures / 15 min →
  429, even with the correct password); success clears the history.
- **3.10**: ticket-create audit entry logs `glpi_ticket_id` only, never the raw
  GLPI response body.

### ✅ Phase 4 — Zabbix / GLPI / authd correctness

- Active alerts now use `problem.get` with `min_severity` + `recent` (5.2)
  instead of `trigger.get` noise.
- Zabbix auto-registration: idempotent `ensure_host_registered` (host.get →
  host.create) so a poller run can't break existing hosts.
- GLPI open-ticket list normalized: the router flattens a pagination envelope
  back to a plain array for the frontend's `.map` (5.8).
- Wazuh manager `AUTH_PASSWORD=${WAZUH_ENROLL_KEY}` wired into compose — authd
  enrollment now actually requires the enroll key the agents carry.

### ✅ Phase 5 — Grafana + MeshCentral made real

- Grafana: missing `GET /dashboards/proxy/{path}` route implemented — serves
  the signed panel-embed URL, proxying same-origin to Grafana with the service
  token (host/credentials never reach the browser).
- MeshCentral: client rewritten to the real API (`X-MeshCentral-AuthToken`,
  `/meshcentral/api/*`, device list/status), returns **portal-host**
  `session_url` (never the internal container name); `meshcentral_public_url`
  setting; nginx gains `/meshcentral/` proxy (with websocket upgrade) and the
  reverse-proxy joins `aaditech-internal` so it can reach MeshCentral.

### ✅ Phase 6 — Dead code removed, readers wired

- Removed the Zabbix disk-forecast placeholder (client + router) and dead
  `send_report_email`.
- Fixed `NameError` in `read_audit_entries` and `read_health_events`
  (`entries` was never initialized) and exposed both via a new
  `app/routers/system.py` (`GET /system/audit`, `GET /system/health`, viewer+).

### ✅ Phase 7 — ILM + poller made real

- `app/ilm.py` (NEW): the documented-but-nonexistent ILM scheduler — a FastAPI
  lifespan background task running purge-expired on `AADITECH_ILM_INTERVAL_SECONDS`
  (6h default, disable via `AADITECH_ILM_DISABLED=1`); `run_purge_cycle()`
  shared by the loop and the HTTP endpoint.
- **5.5**: successful PURGE command completion now transitions the item
  QUARANTINED → PURGED in the store (`mark_item_purged`).
- Poller token is now **self-minted at install**: `install-agent.bat` calls
  `POST /api/agent-installer/self-token` (gated by the enroll key the package
  already carries, constant-time compare) and writes `SERVICE_TOKEN` into
  `AADITECH_ENV.txt`; `agent-command-poller.ps1` falls back to reading that
  file, so the scheduled task needs no hand-pasted arguments.

### ✅ Phase 8 — verification

- Backend **131/131** (was 117; +ILM, +system router, +purge transition,
  +rate limiter, +id_token/security cases). Frontend **12/12**, `vite build`
  green, `docker compose config` exit 0, app module loads clean.

### ⚠️ Still open (unchanged, need live environments)

- Real SMTP/Graph sends, live GitHub build-and-pull, live Windows silent-install.
- `zabbix-server` pre-existing DB-schema mismatch.
...
- ~~Wazuh enrollment: manager `authd` password not yet wired to the enroll key.~~
  (Fixed in session 13 — `AUTH_PASSWORD=${WAZUH_ENROLL_KEY}` on wazuh-manager.)
